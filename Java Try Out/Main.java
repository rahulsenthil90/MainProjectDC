					...
					while(f1){
						...
						while(f2){
							if(ochoice==1){
							    ...
							    File frr=new File("A.txt");
								FileOutputStream fout= new FileOutputStream(frr,true);
								PrintWriter pw=new PrintWriter(fout);
								pw.write("Bill Id      Product Id     Customer name   Product name    Stock  Price");
									pw.close();if(billcount<0){
										pw.write("No bill are Added\n");pw.close();
									}
									else{
										for(i=0;i<=billcount;i++){
											if(b[i].billid!=0){
												pw.write(b[i].billid+"         "+b[i].productid+"          "+b[i].customername+"               "+b[i].productname+"          "+b[i].stock+"      "+((p[b[i].productid-10001].productprice*b[i].stock)+((p[b[i].productid-10001].productprice*b[i].stock)*20/100)));
											pw.close();}
										}
									}
								pw.flush();
